...
def match_urns(section_df):
    ...
    for index, credit_row in credit_rows.iterrows():
        urn = credit_row['URN']
        matching_debit = debit_rows[debit_rows['URN'] == urn]
        if not matching_debit.empty:
            matching_debit = matching_debit.iloc[0]
...
df = df.groupby(['account', 'section_number_FINAL']).apply(match_urns)
...
# Append the generated rows to the original DataFrame
df = df.append(generated_rows)
